...
def read_imageme(scanlist):  # 读取单组图像
    data3d = np.zeros((304, 640, 304), dtype=np.int)
...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader, img1_trans, img2_trans):
        ...
        if isinstance(self._dataset[0], dict):
            self.alllist = self._dataset[0]
        else:
            self.alllist = self._dataset[0][()]  # ndarray转化为内置字典类型dict
        self.Bsf = self._dataset[3]

        self.img3_trans = trans.Resize([224, 224])
        self.img4_trans = trans.Compose([trans.CenterCrop([256, 256]), trans.Resize([224, 224])])
    ...
    #         thedata = []
    #         Blist = alllist['OCT'][idx]  # 查询所有B-scan文件名
    #         for bs in Blist:
    #             thedata.append(torch.from_numpy(Bsf[bs]))  # [256]
    #         thedata = torch.stack(thedata, 0)
    #         data[0].append(thedata)
    #
    #         Flist = alllist['FULL'][idx]
    #         thedata = image_transform(Flist) / 255.
    #         data[1].append(torch.from_numpy(thedata))
    #
    #     data[0] = torch.stack(data[0], 0)
    #     data[0] = data[0].type(torch.FloatTensor)
    #
    #     data[1] = torch.stack(data[1], 0)
    #     data[1] = torch.unsqueeze(data[1], dim=1)
    ...
    def fetch(self, possibly_batched_index):
        ...
        data[3] = copy.deepcopy(data[1])
        data[3] = self.img3_trans(data[3])
        data[3] = data[3].repeat(1, 3, 1, 1)

        data[4] = copy.deepcopy(data[0])
        data[4] = torch.unsqueeze(data[4], dim=1)
        data[4] = self.img4_trans(data[4])
        data[4] = data[4].repeat(1, 3, 1, 1)

        return data  # 不转第8步  转回第6步再转回第2步再转回第一步，返回主程序

    # 迭代第二步 dataloader.py 473
    def _next_data(self):
        index = self._next_index()  # may raise StopIteration   转迭代第三步  传递的是图片路径
...
class BatchSampler(Sampler[List[int]]):
    ...
    def __init__(self, sampler: Sampler[int], batch_size: int, iter_num: int, drop_last: bool, dataset) -> None:
...
class DataLoader2(Generic[T_co]):
    ...
    def __init__(self, dataset, batch_size: Optional[int] = 1, iter_num: int = 10,
        ...
        if splfun == 'rand':
            sampler = RandomSampler(dataset[1], generator=generator)  # 初始化RandomSampler
        else:
            sampler = SequentialSampler(dataset[1])

        batch_sampler = BatchSampler(sampler, batch_size, iter_num, drop_last, dataset)  # 初始化BatchSampler
