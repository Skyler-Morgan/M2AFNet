...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    ...
    def fetch(self, possibly_batched_index):
        ...
        for idx in possibly_batched_index:
            thedata = self._dataset[0][0, :, :, idx]/255.
            thedata = thedata.copy()
            data[1].append(self.img2_trans(thedata))
            thedata = self.img2_trans(thedata)
            if torch.rand(1) < 0.5: #随机横向翻转
                thedata = torch.flip(thedata, [2])
            if torch.rand(1) < 0.5: #随机纵向翻转
                thedata = torch.flip(thedata, [1])
...
class BatchSampler_clbl(Sampler[List[int]]):
    ...
    def __iter__(self):
        ...
        #对每一类从clist随机采样两个编码
        for i in range(int(self.batch_size/4*200)):
            # 随机打乱
...
class RandomSampler(Sampler[int]):
    ...
    def __init__(self, data_len: int, replacement: bool = False,
...
class DataLoader(Generic[T_co]):
    ...
    def __init__(self, dataset, batch_size: Optional[int] = 1,
        ...
        if spmod == 'clbl':
            sampler = None
            batch_sampler = BatchSampler_clbl(sampler, batch_size, drop_last, dataset)  # 初始化BatchSampler
        else:
            if spmod == 'seq':
                sampler = SequentialSampler(len(dataset[1]))
            else:
                sampler = RandomSampler(len(dataset[1]))  # 初始化RandomSampler
            batch_sampler = BatchSampler(sampler, batch_size, drop_last)  # 初始化BatchSampler
